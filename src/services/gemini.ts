...
const SYSTEM_INSTRUCTIONS: Record<AgentType, string> = {
  ...
  speaking: "You are an IELTS Speaking tutor. Maintain natural conversation flow and only provide feedback when explicitly requested at the end of the session with the signal \"The session has finished please give feedback in json\".",
...
export class GeminiService {
  ...
  private constructor() {
    this.genAI = new GoogleGenerativeAI(process.env.LEARNLM_API_KEY || '');
  ...
  async sendMessage(
    ...
    try {
      ...
      if (systemInstruction) {
        ...
        await this.chat.sendMessage(SYSTEM_INSTRUCTIONS[agentType]);
      }
